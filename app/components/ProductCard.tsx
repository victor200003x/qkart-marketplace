...
export default function ProductCard({ product, offers, shops }: ProductCardProps) {
  ...
  return (
    <div className="group relative overflow-hidden rounded-[2rem] border border-white/20 bg-white/10 p-5 shadow-[0_20px_60px_-30px_rgba(15,23,42,0.4)] backdrop-blur-[20px] transition-all duration-300 hover:-translate-y-1 hover:backdrop-blur-[30px] hover:shadow-[0_30px_90px_-30px_rgba(15,23,42,0.45)]">
      ...
      <div className="mb-3 flex flex-wrap items-center gap-2 text-sm text-slate-500">
        {product.aisle && <span className="rounded-full bg-slate-100/80 px-3 py-1 text-xs font-semibold uppercase tracking-[0.2em] text-slate-700">{product.aisle}</span>}
        <span>{product.category} / {product.subcategory}</span>
      </div>
      <p className="text-sm text-slate-600 mb-4">{product.description}</p>
      <div className="mb-4 rounded-3xl border border-white/10 bg-slate-100/5 p-4 text-sm text-slate-300">
        <p className="font-semibold text-slate-100">Best available offer</p>
        {bestOffer ? (
